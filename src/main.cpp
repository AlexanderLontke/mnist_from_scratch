...
int main() {
    ...
    auto ffnn = std::make_unique<FeedForwardNeuralNetwork>(784, 512, 10, 0.01f);
    ...
    auto model_trainer = std::make_unique<ModelTrainer<Matrix>>(
        *ffnn, *cross_entropy_loss, *train_dataloader, *test_dataloader, 200, *multiclass_accuracy, 128
    );
